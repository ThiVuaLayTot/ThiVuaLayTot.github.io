...
<i>Nếu có vấn đề thì xin hãy liên hệ <a href="/leaders#admins" target="_top">quản trị viên</a>.</i>
    ...
    <script>
        ...
        // Cache dữ liệu player
        const playerCache = new Map();
        
        // Rate limiting - max 10 requests cùng lúc
        ...
        async function getCTTQTournamentIds(monthId) {
            try {
                const url = `${GIST_TOURNAMENTS}/${monthId}.txt`;
                const text = await fetchText(url);
                return text ? text.split('\n').filter(line => line.trim()) : [];
            } catch (error) {
                console.error(`Error fetching tournaments for ${monthId}:`, error);
                return [];
            }
        }
        ...
        function parsePlayerData(playerData) {
            if (!playerData) {
                return { username: 'unknown', avatar: 'https://www.chess.com/bundles/web/images/user-image.007dad08.svg' };
            }
        ...
        async function getTournamentTopPlayers(tourId) {
            ...
            const players = [];
            const points = [];
            // Lấy tất cả players, không giới hạn
        ...
        async function aggregateMonthlyScores(monthId) {
            ...
            // Fetch tournament data song song
            ...
            // Fetch tournament top players song song
            ...
            for (let i = 0; i < tourIds.length; i++) {
                ...
                if (players.length > 0 && tournamentData) {
                    ...
                    // Tính tổng điểm cho tất cả players
        ...
        async function getMonthlyTop6(monthId) {
            ...
            // Chỉ fetch dữ liệu cho top 6 cuối cùng
            ...
            // Kiểm tra cheater chỉ cho top 6
                ...
                return `
                    <div class="cttq-card">
                        ...
                        <div class="cttq-card-tournaments">
                            <button class="cttq-toggle-btn" onclick="const list = this.nextElementSibling; list.classList.toggle('show'); this.innerHTML = list.classList.contains('show') ? '▼ Ẩn' : '▶ Xem (' + ${playerTournaments.length} + ')';">
                                ▶ Xem (${playerTournaments.length})
                            </button>
        ...
        async function renderMonthTable(monthId) {
            ...
            try {
                ...
                document.getElementById(`cttq-stats-${monthId}`).innerHTML = `👥 ${totalPlayers} người | 🏆 ${tournaments.length} giải`;
                ...
                if (topPlayers.length === 0) {
                    ...
                    for (let i = 0; i < topPlayers.length; i++) {
                        ...
                        const parsed = parsePlayerData(top6Data[i]);
                        const isCheater = cheaterUsernames.has(player.username.toLowerCase());
                        const cheaterIcon = isCheater ? ' <span style="color: #f87171;">⚠️</span>' : '';
                        const playerTournaments = getPlayerTournamentsInMonth(player.username, tournaments);
                ...
                console.error(`❌ Error rendering ${monthId}:`, error);
                document.getElementById(`cttq-tbody-${monthId}`).innerHTML = '<tr><td colspan="3" style="text-align: center; padding: 20px; color: #f87171;">❌ Lỗi: ' + error.message + '</td></tr>';
            }
        }
